...
//const char* ssid = "CESARI_2.4GHz"; // Casa
//const char* password = "m8tu232905";
const char* ssid = "Hongos"; 
const char* password = "Pleurotus_01";
WiFiClient espClient;

// Configuración del broker MQTT
const char* mqttServer = "micesari.servehttp.com";
...
OneWire ourWire(4);                // DS18B20
DallasTemperature ds18b20(&ourWire);

const int DHT_PIN = 2; // DHT11
DHTesp dhtSensor;
...
void conectarMQTT() {
  while (!client.connected()) {
    ...
    if (client.connect("WemosClient", mqttUser, mqttPassword)) {
      ...
      client.subscribe("hongo/iniProce");
    } else {
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (String(topic) == "hongo/iniProce") {
    if (message == "true") {
      monitorear = true;
...
void publicarDatos() {
  ds18b20.requestTemperatures(); //Se envía el comando para leer la temperatura del hongo
  float tempDS18B20 = ds18b20.getTempCByIndex(0); //Se obtiene la temperatura en ºC del hongo
  client.publish("hongo/tempSonda", String(tempDS18B20,2).c_str());

  TempAndHumidity data = dhtSensor.getTempAndHumidity();
  client.publish("hongo/tempAmb", String(data.temperature).c_str());
  client.publish("hongo/humAmb", String(data.humidity).c_str());
  ...
  client.publish("hongo/co2", String(ppmCO).c_str());
